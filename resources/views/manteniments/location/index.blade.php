          ...
          <table id="example2" class="table table-bordered table-hover dataTable" role="grid" aria-describedby="example2_info">
            ...
            @foreach ($locations as $location)
                <tr role="row" class="odd">
                  ...
                  <td>
                    <form class="row" method="POST" action="{{ route('location.destroy', ['id' => location->id]) }}" onsubmit = "return confirm('Estas segur d'esborrar?')">
                        <input type="hidden" name="_method" value="DELETE">
                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                        <a href="{{ route('location.edit', ['id' => location->id]) }}" class="btn btn-warning col-sm-3 col-xs-5 btn-margin">
                        Editar
                        </a>
